...
System.out.println("In CHanged password1 :: "+uname);
try {
	...
	System.out.println("In CHanged password2 :: "+uname);
	//required for db connection
	...
	*/
	...
	rs = null;
	rs = st.executeQuery("SELECT password, tempPwd  FROM user where uname='" + uname + "'");
	while (rs.next()) {
		rpasswd = rs.getString(1);
		tempPwd = rs.getString(2);
	}
	System.out.println("DB password:" + rpasswd);
	System.out.println("DB temp password:" + tempPwd);
	System.out.println("unHashedCurrent password:" + unHashedCurrPwd);
	*/
	if (true){//rpasswd.equals(currPwd) ||tempPwd.equals(currPwd) ) {//if passwords match
		//gen new salt						
